...
public class HorarioController {
    ...
    @GetMapping("/listar/{iddoctor}")
    @ApiResponse(code = 200, message = "OK")
    public ResponseEntity<Object> getHorarios(@PathVariable Long iddoctor) {
        try {
            List<HorariosDoctorDto> c = horarioService.obtenerHorariosPorIdDoctor(iddoctor);
            return ResponseHandler.generateResponse("Datos listados los horarios por doctor  con exito!.", HttpStatus.OK, c);
        } catch (Exception e) {
            return ResponseHandler.generateResponse(e.getMessage(), HttpStatus.MULTI_STATUS, null);
        }
    }
